...
A limited amount of user batch processing might be available via the RAC for Data Preview 2 (DR2).
...
and might increase for Data Preview 2 (with further increases over the ten-year survey likely) [#f3]_.

The total shared disk space for batch users to be allocated by the RAC remains TBD.
...
The size of a dataset retrieved by a query and held in memory depends on the Notebook Aspect server size that is selected by the user.
